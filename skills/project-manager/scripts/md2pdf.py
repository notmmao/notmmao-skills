...
from reportlab.platypus import (
    ...
    Table, TableStyle, NextPageTemplate, Flowable, KeepInFrame, Image
)
from reportlab.pdfbase import pdfmetrics
from reportlab.pdfbase.ttfonts import TTFont

# SVG support - disabled by default (can be problematic with complex SVGs)
...
def esc_code(text):
    """Escape for code blocks: preserve indentation, line breaks, and ALL spaces (for ASCII art)."""
    out = []
    for line in text.split('\n'):
        e = esc(line)
        # Preserve ALL spaces in code blocks for ASCII art alignment
        e = e.replace(' ', '&nbsp;')
        out.append(e)
    return '<br/>'.join(out)
...
class PDFBuilder:
    def __init__(self, config):
        ...
        # H2 标题配置：是否换页、留白比例（相对于 body_h）
        self.h2_page_break = config.get("h2_page_break", True)
        self.h2_space_before = config.get("h2_space_before", 0.15)  # 默认 15%
        self.h3_space_before = config.get("h3_space_before", 3*mm)
        self.h3_space_after = config.get("h3_space_after", 1*mm)
    ...
    def parse_md(self, md):
        ...
        code_max = self.cfg.get("code_max_lines", 200)
        ...
        while i < len(lines):
            ...
            if stripped.startswith('```'):
                if in_code:
                    ...
                    if ct.strip():
                        ...
                        # Wrap in KeepInFrame to handle content that's too large for a page
                        wrapped = KeepInFrame(0, 0, [para], mode='shrink')

                        if self._code_style_type == "border":
                            # For border style, we need to wrap the Para first, then add border
                            # Use a simpler approach: just add the paragraph without border for very long blocks
            ...
            # Inline image: ![alt](path) or ![alt](path "title")
            img_match = re.match(r'^!\[(.+?)\]\((.+?)(?:\s+"(.+?)")?\)$', stripped)
            if img_match:
                alt_text = img_match.group(1)
                img_path = img_match.group(2)
                # Resolve path relative to input file
                input_dir = os.path.dirname(self.cfg.get("input_path", ""))
                full_path = os.path.join(input_dir, img_path) if input_dir else img_path
                ext = os.path.splitext(full_path)[1].lower()

                # Skip SVG files (not supported by reportlab directly)
                if ext == '.svg':
                    print(f"Info: Skipping SVG {img_path} (use PNG/JPG instead)", file=sys.stderr)
                    story.append(Paragraph(f"<i>[图: {md_inline(alt_text, ah)}]</i>", ST['body']))
                    i += 1; continue
                ...
                # Fallback: render as paragraph with alt text
                story.append(Paragraph(f"<i>[Image: {md_inline(alt_text, ah)}]</i>", ST['body']))
                i += 1; continue
            ...
            if stripped.startswith('## '):
                ...
                if self.h2_page_break:
                    story.append(PageBreak())
                cm = ChapterMark(title, level=1); story.append(cm)
                hdeco = self.L["heading_decoration"]
                story.append(Spacer(1, self.body_h * self.h2_space_before))
            ...
            # H3 = Section
            if stripped.startswith('### '):
                story.append(Spacer(1, self.h3_space_before))
                story.append(Paragraph(md_inline(stripped[4:].strip(), ah), ST['h3']))
                story.append(Spacer(1, self.h3_space_after))
                i += 1; continue

            # H4-H6 = Subsection (same style as H3)
            if stripped.startswith('####') or stripped.startswith('#####') or stripped.startswith('######'):
                # Count # and extract title
                match = re.match(r'^(#{4,6})\s+(.+)', stripped)
                title = match.group(2) if match else stripped.lstrip('#').strip()
                story.append(Spacer(1, 2*mm))
                story.append(Paragraph(md_inline(title, ah), ST['h3']))
                i += 1; continue
            ...
            # Bullets (with indent level detection)
            if re.match(r'^(\s*)[-*]\s', line):
                # Calculate indent level (2 spaces = 1 level)
                indent_match = re.match(r'^(\s*)[-*]\s', line)
                indent_spaces = len(indent_match.group(1)) if indent_match else 0
                indent_level = max(0, (indent_spaces + 1) // 2)  # 0, 1, 2, ...

                # Base indent + per-level indent (18pt base + 12pt per level)
                left_indent = 18 + indent_level * 14
                bullet_indent = 6 if indent_level == 0 else 6 + indent_level * 2

                # Create dynamic style for this indent level
                bullet_style = ParagraphStyle(
                    f'Bul_L{indent_level}',
                    parent=ST['bullet'],
                ...
                content = content_match.group(1) if content_match else stripped[2:].strip()

                story.append(Paragraph(f"\u2022  {md_inline(content, ah)}", bullet_style))
                i += 1; continue

            # Numbered list (with indent level detection)
            m = re.match(r'^(\s*)(\d+)\.\s+(.+)', line)
            if m:
                # Calculate indent level
                indent_spaces = len(m.group(1))
                indent_level = max(0, (indent_spaces + 1) // 2)

                # Base indent + per-level indent
                left_indent = 18 + indent_level * 14
                number_indent = 6 if indent_level == 0 else 6 + indent_level * 2

                # Create dynamic style
                number_style = ParagraphStyle(
                    f'Num_L{indent_level}',
                    parent=ST['bullet'],
                    leftIndent=left_indent,
                    bulletIndent=number_indent,
                )

                story.append(Paragraph(f"{m.group(2)}.  {md_inline(m.group(3), ah)}", number_style))
                i += 1; continue
...
def main():
    ...
    parser.add_argument("--code-max-lines", default=200, type=int, help="Max lines per code block before truncation")
    parser.add_argument("--h2-page-break", action='store_true', default=True, help="H2 headings start on new page (default: True)")
    parser.add_argument("--no-h2-page-break", dest='h2_page_break', action='store_false', help="H2 headings do NOT start on new page")
    parser.add_argument("--h2-space-before", default=0.15, type=float, help="Space before H2 as ratio of body height (default: 0.15)")
    parser.add_argument("--h3-space-before", default=3, type=float, help="Space before H3 in mm (default: 3)")
    parser.add_argument("--h3-space-after", default=1, type=float, help="Space after H3 in mm (default: 1)")
    ...
    if args.list_themes:
        ...
        for name in THEMES.keys():
            t = THEMES[name]
            layout_desc = t.get("layout", {})
            print(f"  {name}")
            if layout_desc:
                cover = layout_desc.get("cover_style", "centered")
                header = layout_desc.get("header_style", "full")
                print(f"    Cover: {cover}, Header: {header}")
        return

    # Require --input when not listing themes
    if not args.input:
        parser.error("--input is required (unless using --list-themes)")
    ...
    config = {
        ...
        "h2_page_break": args.h2_page_break,
        "h2_space_before": args.h2_space_before,
        "h3_space_before": args.h3_space_before * mm,
        "h3_space_after": args.h3_space_after * mm,
